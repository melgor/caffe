...
/**
 * @brief Compare value of vector at bottom. If are the same, return 1, otherwise 0.
 *        
 *
 * NOTE: does not implement Backwards operation.
 */
template <typename Dtype>
class SimLayer : public Layer<Dtype> {
 public:
  explicit SimLayer(const LayerParameter& param)
      : Layer<Dtype>(param) {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  virtual inline const char* type() const { return "Sim"; }
